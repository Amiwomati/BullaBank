...
describe('formatCurrency', () => {
  ...
  test('should throw error for non-numeric input', () => {
    ...
    expect(() => formatCurrency(undefined)).toThrow('El monto debe ser un número');
  });
...
describe('calculateNewBalance', () => {
  ...
  test('should handle decimal amounts', () => {
    expect(calculateNewBalance(1000.50, 500.25)).toBe(500.25);
  });

  test('should throw error for non-numeric inputs', () => {
    expect(() => calculateNewBalance('1000', 500)).toThrow('Los montos deben ser números válidos');
    expect(() => calculateNewBalance(1000, '500')).toThrow('Los montos deben ser números válidos');
    expect(() => calculateNewBalance(null, 500)).toThrow('Los montos deben ser números válidos');
  });
